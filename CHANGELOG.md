...
## 版本 0.3.0 - 正文质量增强版（2026-06-12）

### 新增功能

#### 1. `novel extend` 命令 - 连载续写

基于已写正文续写章纲，无需等待参考小说更新。

```bash
novel extend my-novel --volume 1 --chapters 20
```

流程：用已写章节替代参考小说批次摘要 → 生成新批次摘要 → 生成新章纲 → 可继续 `novel write`。

**相关文件**：`training/adaptive_builder.py` (`gen_extend_outlines`)、`novel_cli.py`

---

#### 2. 正文自检与修订接入

生成正文后自动执行 18 项规则自检，有违规则自动修订，失败不影响落盘。

输出示例：
```
-> 正在自检第3章...
-> 发现 2 项违规，正在修订...
-> 修订完成：删除多余总结段落，修正视角漂移
```
...
生成每章正文时，按比例从参考小说中取对应位置章节的前 1500 字作为风格示例，注入 context。比纯提示词描述更有效地保持文风。

---

#### 4. 风格强度档位细化（11档）

从原5档细化为每10%一档，共11个档位，每档有独立描述：
...
- **批次文件范围匹配**：`gen_serial_chapters` 查找批次文件改为范围匹配，修复 `novel extend` 后批次文件名不一致导致加载失败的问题。

---

### 修改的文件

- `core/style_intensity.py` - 风格强度11档细化
- `training/adaptive_builder.py` - self_check 接入、原著示例注入、`gen_extend_outlines` 函数、批次文件范围匹配修复
- `novel_cli.py` - 新增 `extend` 命令
